...
class Result extends Component {
  ...
  render() {
    ...
    const { reviews, showReviews, firstLoad } = this.state;
    ...
    return (
      <li key={id} className="results__item">
          ...
          <div className="results__item-info">
            <div className="results__item-header">
              <div className="results__item-left">
                ...
                <h3 className="results__item-city">
                  {country} - {city}{" "}
                </h3>
              </div>
            ...
            <div className="results__item-bottom">
              <Buttons action={() => this.showReviews(id)}>
                {showReviews ? "Hide reviews" : "Show reviews"}
              </Buttons>
              <div className="results__item-price-wrapper">
                <div className="results__item-price">{price}&euro;</div>
        ...
        <div className="reviews">
          {showReviews &&
            reviews.map((review, index) => <Reviews {...review} key={index} />)}
          {showReviews && reviews.length === 0 && firstLoad && <NoReviews />}
        </div>
      </li>
    );
  }
